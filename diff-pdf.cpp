                             ...
                             int offset_x = 0, int offset_y = 0)
{
    ...
    Rect rdiff = {
        std::min(r1.x, r2.x), std::min(r1.y, r2.y),
        std::max(r1.x + r1.width, r2.x + r2.width), std::max(r1.y + r1.height, r2.y + r2.height)
    };
    r1.x -= rdiff.x;
    r1.y -= rdiff.y;
    r2.x -= rdiff.x;
...
int usage()
{
    fprintf(stdout, R"HELP(Usage: diff-pdf [-h] [-v] [-s] [-m] [--output-diff <str>] [--channel-tolerance <num>] file1.pdf file2.pdf
...
int main(int argc, char *argv[])
{
    ...
    if ( !doc1 )
    {
        fprintf(stderr, "Error opening %s: %s\n", (const char*) file1.c_str(), err->message);
        g_error_free(err);
        return 3;
    }
    g_free(uri1);
    ...
    if ( !doc2 )
    {
        fprintf(stderr, "Error opening %s: %s\n", (const char*) file2.c_str(), err->message);
        g_error_free(err);
        return 3;
    }
    g_free(uri2);
    ...
    {
        gchar 
        retval = doc_compare(doc1, doc2, pdf_file.c_str(), NULL) ? 0 : 1;
    }
    else
    {
